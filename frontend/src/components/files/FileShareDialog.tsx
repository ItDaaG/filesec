...
export function FileShareDialog({ file, open, onOpenChange }: FileShareDialogProps) {
  ...
  const shareMutation = useMutation({
    mutationFn: (emails: string[]) => shareFile(file.id, emails),
    onSuccess: () => {
      setEmailInput("");
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.filePermissions(file.id) });
    },
  });
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="file-share-emails">Invite by email</Label>
            <div className="flex gap-2">
              <Input
                id="file-share-emails"
